...
class sssp: public graphlab::ivertex_program<graph_type, min_distance_type>,
...
public:
	...
	min_distance_type gather(icontext_type& context, const vertex_type& vertex,
		...
		return min_distance_type(edge.source().data().dist + edge.data().dist);
	}

	void apply(icontext_type& context, vertex_type& vertex,
			const gather_type& total) {

		if (vertex.id() == SOURCE && vertex.data().dist != 0) {
			vertex.data().dist = 0;
			changed = true;
			return;
		}
